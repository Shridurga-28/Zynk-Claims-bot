...
def gpt_generate_chat_from_text(text: str) -> str:
    ...
    if text.lower().strip() in ["hi", "hello", "hey"]:
        return "👋 Hello! I’m EasyClaim Bot. I can help you file or understand insurance claims. You can upload a bill, ask about claim status, or type your question below!"

    prompt = (
        "You are an insurance claims assistant cum a helpful insurance bot. If the following data is normal chat, then chat with them. Or, if it is OCR text from a claim invoice, then,\n\n"
        "Your task:\n"
